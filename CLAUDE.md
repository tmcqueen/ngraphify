...
- `src/Ngraphiphy/` — core: extraction (9 languages), graph build, dedup, cluster, analysis, report, cache
- `src/Ngraphiphy.Llm/` — MAF 1.5.0 agent; 5 providers (OpenAI, Anthropic, Ollama, Copilot, A2A)
- `src/Ngraphiphy.Pipeline/` — `RepositoryAnalysis.RunAsync` orchestrates the full pipeline
- `src/Ngraphiphy.Cli/` — commands: `analyze`, `report`, `query`, `serve`; MCP server under `Mcp/`
- `tests/` — TUnit 1.43.41; run with `dotnet run --project tests/<Project>/`

## Key facts

- CLI binary name: `ngraphiphy-cli` (not `ngraphiphy` — Linux CLR collision fix)
